...
This boilerplate creates a working Pulumi-owned provider named `netfoundry`.
...
1. Set the following options:
   * Owner: pulumi 
   * Repository name: pulumi-netfoundry-native (replace "netfoundry" with the name of your provider)
   * Description: Pulumi provider for netfoundry
   * Repository type: Public
1. Clone the generated repository.

From the templated repository:

1. Search-replace `netfoundry` with the name of your desired provider.
...
1. A `provider/` folder containing the building and implementation logic
   1. `cmd/`
      1. `pulumi-gen-netfoundry/` - generates language SDKs from the schema
      2. `pulumi-resource-netfoundry/` - holds the package schema, injects the package version, and starts the gRPC server
   2. `pkg`
...
3. `sdk` - holds the generated code libraries created by `pulumi-gen-netfoundry/main.go`
4. `examples` a folder of Pulumi programs to try locally and/or use in CI.
5. A `Makefile` and this `README`.

### Writing the schema

The [JSON schema file](https://www.pulumi.com/docs/guides/pulumi-packages/schema) is used by `pulumi-gen-netfoundry` to create language-specific SDKs. 
...
This repository provides the [netfoundry example schema](./provider/cmd/pulumi-resource-netfoundry/schema.json) to get you started.  
